...
export class DealsFormComponent extends ImportCSVComponent implements OnInit, OnChanges 
{
		...
		protected formbuilder: FormBuilder, protected dialog: MatDialog) 
	{
		...
		this.Form = this.formbuilder.group({
			...
			rules_of_purchase: [null, [Validators.required]],
		...
		if(this.is_child == false)
        {
			...
			this.sub = this._route.params.subscribe(params => {
				...
				if (this.id != 'add') {
					...
					this.Offer = JSON.parse(localStorage.getItem('Deal'));
	...
	private _filter(name: string): any[] 
	{
		...
		return this.Outlets.filter(option => option.name.toLowerCase().indexOf(filterValue) === 0);
	}
	...
	getImage(item): any 
	{
		if (this.id != 'add') 
		{
			return appConfig.file_url + this.Offer[item];
		}
		else 
		{
			return '';
