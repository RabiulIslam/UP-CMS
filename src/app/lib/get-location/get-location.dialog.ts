...
export class GetLocationDialog implements OnInit, AfterViewInit
{
    ...
    ngOnInit(): void
    {
        ...
        this.formatteAddress(loc, (location) => {
    ...
    markerDragEnd(m, $event: any)
    {
        ...
        this.gMaps.panTo(this.currentLocation);
        this.formatteAddress(this.currentLocation, (location) => {
            this.formatted_address = location;
        });
    }
    ...
    formatteAddress(location, callback) {
        ...
        this._loader.load().then(() =>
        {
            ...
            geoCoder.geocode({ 'location': location }, (responses, status) => {
    ...
    makeFormattedAddress(responses): string
    {
        let address = null;
        if (responses && responses.length > 0)
        {
            ...
            responses[0].address_components.forEach((value, index) =>
            {
