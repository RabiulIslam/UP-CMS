<?php
    ...
    class outh{
        ...
        static function methodParamscheck(){
            ...
            if($checkIp == "blocked"){
                $response = ["status" => 403, "message" => 'Forbidden', "data" => null];
            ...
            if($methodName == '' || $method == "OPTIONS"){
                $response = ["status" => 200, "message" => 'Wellcome to UP', "data" => null];
                echo json_encode($response);
                header("HTTP/1.1 200");
                exit();
            }
            ...
            //print_r(self::$params);
            return ["methodName" => $methodName, "method" => $method, "params" => self::$params];
        }
        /* :------------------------------outhentication check  ------------------------------*/
        static function outhforAll($methodName, $con, $authorization){
            $withoutsessionarray = ["signIn", "checkEmail", "checkPhone", "forgotPassword", "changePassword"];
            $bothwithorwithout = ["addUser"];
            ...
            if($error != ""){
                if($error == '2'){
                    ...
                    $response = ["status" => 401, "message" => 'Invalid Parameter: Headers', "data" => null];
                }
                echo json_encode($response);
                header("HTTP/1.1 401");
                exit();
