...
/* 1:------------------------------method start here addAdmin ------------------------------*/
   static function addAdmin($params) 
	{
		$con =$params['dbconnection'];	
		if($emailexist=DbMethods::checkEmail($params)=='emailexist') return  'emailexist';
		if($phoneexist=DbMethods::checkPhone($params)=='phoneexist') return  'phoneexist';
		$params['password']=sha1($params['password']);
   ...
   static function getAdmins($params) 
	{
		...
		if($adminsCount !='')
		return array("admin"=>$admin ,"adminsCount"=>$adminsCount ) ;
		else return '';
	}	
...
static function updateCategory($params)
 {
	...
	if($params['image']!=""){   $image=" `image`='{$params['image']}' ,";  }
...
static function getCategories($params)
 {
	...
	if($categoriesCount !='')
	return array("categories"=>$categories ,"categoriesCount"=>$categoriesCount ) ;
	else return '';
 }
...
static function addMerchant($params)
 {
	...
	if($params['name']!=""){   $name=" `name`='{$params['name']}' ,";  }
	if($params['title']!=""){   $title=" `title`='{$params['title']}' ,";  }
	if($params['phone']!=""){   $phone=" `phone`='{$params['phone']}' ,";  }
	if($params['email']!=""){   $email=" `email`='{$params['email']}' ,";  }
	if($params['gender']!=""){   $gender=" `gender`='{$params['gender']}' ,";  }
	if($params['point_of_contact']!=""){   $point_of_contact=" `point_of_contact`='{$params['point_of_contact']}' ,";  }
	if($params['up_account_manager']!=""){   $up_account_manager=" `up_account_manager`='{$params['up_account_manager']}' ,";  }
	if($params['contract_start_date']!=""){   $contract_start_date=" `contract_start_date`='{$params['contract_start_date']}' ,";  }
	if($params['contract_expiry_date']!=""){   $contract_expiry_date=" `contract_expiry_date`='{$params['contract_expiry_date']}' ,";  }
	if($params['notes']!=""){   $notes=" `notes`='{$params['notes']}' ,";  }
...
static function updateMerchant($params)
{
	...
	if($params['contract_expiry_date']!=""){   $contract_expiry_date=" `contract_expiry_date`='{$params['contract_expiry_date']}' ,";  }
	if($params['notes']!=""){   $notes=" `notes`='{$params['notes']}' ,";  }
	if($params['TAC_accepted']!=""){   $TAC_accepted=" `TAC_accepted`='{$params['TAC_accepted']}' ,";  }
...
static function getMerchants($params)
 {
	...
	if($merchantsCount !='')
	return array("merchants"=>$merchants ,"merchantsCount"=>$merchantsCount ) ;
	else return '';
 }
...
static function updateOutlet($params)
{
	...
	//if($outletexist=DbMethods::checkOutlet($params)=='outletexist') return  'outletexist';
	if($params['merchant_id']!=""){   $merchant_id=" `merchant_id`='{$params['merchant_id']}' ,";  }
	if($params['name']!=""){   $name=" `name`='{$params['name']}' ,";  }
	if($params['emails']!=""){   $emails=" `emails`='{$params['emails']}' ,";  }
	if($params['search_tags']!=""){   $search_tags=" `search_tags`='{$params['search_tags']}' ,"; }
	if($params['phone']!=""){   $phone=" `phone`='{$params['phone']}' ,";  }
	if($params['SKU']!=""){   $SKU=" `SKU`='{$params['SKU']}' ,";  }
	if($params['pin']!=""){   $pin=" `pin`='{$params['pin']}' ,";  }
	if($params['address']!=""){   $address=" `address`='{$params['address']}' ,";  }
	if($params['description']!=""){   $description=" `description`='{$params['description']}' ,";  }
	if($params['timings']!=""){   $timings=" `timings`='{$params['timings']}' ,";  }
	if($params['latitude']!=""){   $latitude=" `latitude`='{$params['latitude']}' ,";  }
	if($params['longitude']!=""){   $longitude=" `longitude`='{$params['longitude']}' ,";  }
	if($params['logo']!=""){   $logo=" `logo`='{$params['logo']}' ,";  }
	if($params['image']!=""){   $image=" `image`='{$params['image']}' ,";  }
	if($params['type']!=""){   $type=" `type`='{$params['type']}' ,";  }
	if($params['special']!=""){   $special=" `special`='{$params['special']}' ,";  }
...
static function getOutlets($params)
  {
	...
	if($outletsCount !='')
	return array("outlets"=>$outlets ,"outletsCount"=>$outletsCount ) ;
	else return '';
}
...
static function addOffer($params)
{
	 ...
	 $price="";
	 $special_price="";
	 if($special_price=DbMethods::checkOffer($params)=='offertexist') return  'offertexist';
	
	 if($params['redemptions']!=""){   $redemptions=" `redemptions`='{$params['redemptions']}' ,";  }
	 if($params['per_user']!=""){   $per_user=" `per_user`='{$params['per_user']}' ,";  }
	 if($params['special']=='1') $special_type="`special_type`='{$params['special_type']}' ,";
	 if($params['special']=='1') $special="`special`='{$params['special']}' ,";
	 if($params['renew']=='1') $renew="`renew`='{$params['renew']}' ,";
	 if($params['price']!=""){   $price=" `price`='{$params['price']}' ,";  }
	 if($params['special_price']!=""){   $special_price=" `special_price`='{$params['special_price']}' ,";  }
...
static function addOffers($params)
{
	       $con =$params['dbconnection'];	
			...
			if($titles!='' )
			{
				...
				return array("msg"=>'exists' ,"title"=>$titles ) ;
			}
			for($i=0;$i<count($offers); $i++)
			{
...
static function updateOffer($params)
	{
		...
		$renew='';
		$redemptions='';
		if($offertexist=DbMethods::checkOffer($params)=='offertexist') return  'offertexist';
		...
		if($params['outlet_id']!=""){   $outlet_id=" `outlet_id`='{$params['outlet_id']}' ,";  }
		if($params['title']!=""){   $title=" `title`='{$params['title']}' ,";  }
		if($params['SKU']!=""){   $SKU=" `SKU`='{$params['SKU']}' ,";  }
		if($params['search_tags']!=""){   $search_tags=" `search_tags`='{$params['search_tags']}' ,";  }
		if($params['image']!=""){   $image=" `image`='{$params['image']}' ,";  }
		if($params['start_datetime']!=""){   $start_datetime=" `start_datetime`='{$params['start_datetime']}' ,";  }
		if($params['end_datetime']!=""){   $end_datetime=" `end_datetime`='{$params['end_datetime']}' ,";  }
		if($params['price']!=""){   $price=" `price`='{$params['price']}' ,";  }
		if($params['special_price']!=""){   $special_price=" `special_price`='{$params['special_price']}' ,";  }
		if($params['approx_saving']!=""){   $approx_saving=" `approx_saving`='{$params['approx_saving']}' ,";  }
		if($params['valid_for']!=""){   $valid_for=" `valid_for`='{$params['valid_for']}' ,";  }
		if($params['special']!=""){   $special=" `special`='{$params['special']}' ,";  }
		if($params['special_type']!=""){   $special_type=" `special_type`='{$params['special_type']}' ,";  }
		if($params['renew']!=""){   $renew=" `renew`='{$params['renew']}' ,";  }
		if($params['redemptions']!=""){   $redemptions=" `redemptions`='{$params['redemptions']}' ,";  }
		if($params['description']!=""){   $description=" `description`='{$params['description']}' ,";  }
		if($params['rules_of_purchase']!=""){   $rules_of_purchase=" `rules_of_purchase`='{$params['rules_of_purchase']}' ,";  }
...
static function updateMultipleOffer($params)
	{
		...
		if($params['start_datetime']!=""){   $start_datetime=" `start_datetime`='{$params['start_datetime']}' ,";  }
		if($params['end_datetime']!=""){   $end_datetime=" `end_datetime`='{$params['end_datetime']}' ,";  }
		if($params['active']!=""){   $active=" `active`='{$params['active']}' ,";  }
...
static function getOffers($params)
 {
	...
	if($offersCount !='')
	return array("offers"=>$offers ,"offersCount"=>$offersCount ) ;
	else return '';} 
...
static function getOfferDetail($params)
  {
	...
	if (mysqli_num_rows($result) > 0) 
	{
		...
		if(empty(DbMethods::checkImageSrc($row['image']))) $row['image']='';
		if(empty(DbMethods::checkImageSrc($row['logo']))) $row['logo']='';
		if($row['favourite_id']==NULL) $row['favourite_id']='0'; else $row['favourite_id']='1';
...
static function addNotification($params)
{
	...
	if($params['audience']== 'specificUsers' || $params['audience']== 'specificusers')
	{
		   ...
		   if($params['specificUsers']!=""){   $params['specificUsers']=" `specificUsers`='{$params['specificUsers']}' ,"; }
	}
	elseif( $params['audience']== 'userCreatedDate') 
	{
		if($params['dates']!=""){ $dates=" `dates`='{$params['dates']}' ,";  }
		if($params['dates']=="Both")
		{
			if($params['greater_than']!=""){ $greater_than=" `greater_than`='{$params['greater_than']}' ,";  }
			if($params['less_than']!=""){   $less_than=" `less_than`='{$params['less_than']}' ,";  }
		}
...
static function sendNotification($params) 
{
	...
	elseif($params['audience']=='specificUsers' || $params['audience']== 'specificusers')
	{
	...
	if(!empty($tokens_ios) && count($tokens_ios) > 0)
	{
		...
		foreach ($tokens_ios as $iostokens) {
			...
			DbMethods:: post_async($dir ,array('token'=>$params['token'],'title'=>$params['title'],'message'=>$params['message']
			,'id'=>$params['id'],'deviceType'=>$params['deviceType'],'date'=>$date,'Authorization'=>$params['Authorization']));
...
static function addAccessCode($params)
{
	 ...
	 for($i=0;$i<$params['number']; $i++)
	 {
		...
		{
		 if(DbMethods::checkCode($params)=='codeexist') return  'codeexist';
		 $code=$params['code'];
		}
		$parent_id="";
		if($params['parent_id']!="")
...
static function checkCode($params) 
{
	$con =$params['dbconnection'];	
	$id="";
	if($params['id']!=""){   $id=" AND `id` !='{$params['id']}' ";  }
...
static function updateAccessCode($params)
{
	...
	$subquery="";
	if($params['title']!=""){   $title=" `title`='{$params['title']}' ,";  }
	if($params['code']!=""){   $code=" `code`='{$params['code']}' ,";  }
	if($params['redemptions']!=""){   $redemptions=" `redemptions`='{$params['redemptions']}' ,";  }
	if($params['redeemed']!=""){   $redeemed=" `redeemed`='{$params['redeemed']}' ,";  }
	if($params['expiry_datetime']!=""){   $expiry_datetime=" `expiry_datetime`='{$params['expiry_datetime']}' ,";  }
	if($params['days']!=""){   $days=" `days`='{$params['days']}' ,";  }
	if($params['multiple']!=""){   $multiple=" `multiple`='{$params['multiple']}' ,";  }
...
static function getAccessCodes($params)
 {
	...
	if($accesscodesCount !='')
	return array("accesscodes"=>$accesscodes ,"accesscodesCount"=>$accesscodesCount ) ;
	else return '';
}
...
static function updateUser($params)
  {
	$con =$params['dbconnection'];
	if($params['email'] !='')
	if(DbMethods::checkUserEmail($params)=='emailexist')return  'emailexist';
	if($params['phone'] !='')
	if(DbMethods::checkUserPhone($params)=='phoneexist')return  'phoneexist';
	$name='';
	$email='';
	$phone='';
	$password='';
...
static function getSubscriptions($params)
  {
	...
	$subscriptionsCount="";
	if ($subscriptionsCount = $con->query("SELECT u.`id` FROM  `users` as u 
	INNER JOIN `subscriptions` as s ON(u.`id`=s.`user_id`) WHERE u.`id`=u.`id` ".$where."")) 
	...
	return array("subscriptions"=>$subscriptions ,"subscriptionsCount"=>$subscriptionsCount ) ;
	else return '';
...
static function unsubscribe($params)
  {
	...
	if (mysqli_num_rows($result) > 0) 
	{
		...
		DbMethods:: post_async($dir ,array('user_id'=>$row['user_id'],'phone'=>$row['phone'],'Authorization'=>$row['auth_key']));
...
static function getFavouriteOffers($params)
  {
		...
		return array("usersfavouriteoffers"=>$usersfavouriteoffers ,"usersfavouriteoffersCount"=>$usersfavouriteoffersCount ) ;
		else return '';
  }
...
static function getOrders($params)
  {
		...
		if($allOrdersCount !='')
		return array("allOrders"=>$allOrders ,"allOrdersCount"=>$allOrdersCount ) ;
		else return '';
...
static function getOrderReviews($params)
  {
		...
		*
		...
		if($allReviewsCount !='')
		return array("allReviews"=>$allReviews ,"allReviewsCount"=>$allReviewsCount ) ;
		else return '';
...
static function addUpdateDefault($params)
{
		...
		if($params['uber'] !=''){  $uber=" `uber`='{$params['uber']}' ,";   } 
		if($params['paragraph'] !=''){  $paragraph=" `paragraph`='{$params['paragraph']}' ,";   } 
...
static function updateCreditcardPackages($params) 
{
	...
	$savings="";
	if($params['name']!=''){   $name=" `name`='{$params['name']}' ,";  }
	if($params['qatar_value'] !=''){  $qatar_value=" `qatar_value`='{$params['qatar_value']}' ,";   } 
	if($params['doller_value'] !=''){  $doller_value=" `doller_value`='{$params['doller_value']}' ,";   } 
	if($params['savings'] !=''){  $savings=" `savings`='{$params['savings']}' ,";   } 
...
static function logs($params) 
{
	...
	$con =$params['dbconnection'];
	$excludeitems = array( "apiBasePath","langset", "method","Authorization","url" ,"index","index2","user_deviceType","admin_logstype","table_id"
	,"table_name","admin_id","dbconnection");
